...
// console.log('a',proteinType)
const protein_class = ["hypothetical", "infection", "assembly", "replication", "packaging", "lysis", "regulation", "immune", "integration", "tRNA"]//, "unsorted", "mix"]
// console.log('b',protein_class)
// Labels of rows
// var myVars = [];
d3.csv(props.phageUrl).then(dataphage=> { 
// let data1_a = Array.from(data1);
let myVars = [];
// console.log('data1',dataphage);
dataphage.forEach(d=> {
...
let chooseColor = (d, flag)=>{
  let color = "";
  // when d is protein_class
  if(flag==1){
    switch(d){
    case "assembly": color="#A49ECB"; break;
    case "infection": color="#5D96F8"; break;
    case "hypothetical": color="#93D5CA"; break;
    case "immune": color="#ABABAB"; break;
    case "integration": color="#BF86C0"; break;
    case "replication": color="#4EABDD"; break;
    case "packaging": color="#FDB86A"; break;
    case "lysis": color="#1BF1C2"; break;
    case "tRNA": color="#80CD6E"; break;
    case "regulation": color="#FBB9DA"; break;
...
.enter()
.append("rect")
  .attr("x", function(d) { return -y.bandwidth(); })
  .attr("y", function(d) { return y(d)+y.bandwidth()/4;})
  .style("width", y.bandwidth()/2)
  .style("height", y.bandwidth()/2)
  .style("fill", function(d){return chooseColor(d, 1)});
...
// create a tooltip
// const tooltip = d3.select("#Viz_area")
//     .append("div")
//     .style("opacity", 0)
//     .attr("class", "tooltip")
...
const mousemove = function(md,d) {
  ...
  // console.log(d)
myCol
    .attr("x", Math.floor((md.layerX-2*leftMargin.value)/x.bandwidth())*x.bandwidth()+leftMargin.value)
    .attr("y", 0)
  tooltip
      .style('opacity', 1)
...
// let data_a = Array.from(data);
// console.log(data);
data.forEach(function(d) {
  let x = d.Phage_Acession_ID;
  delete d["Phage_Acession_ID"];
  let cl = _.split(d.protein_sort_class, ';');
  if(cl[0]=='unsorted'){
  }
  else{
...
let cbcount = _.countBy(count, function(n){ return n.x+":"+n.y; });
// console.log(cbcount)
// create a list 'cbdata' in the format {Phage_Acession_ID: the number}
let cbdata = _.countBy(count, function(n){ return n.x; });
...
final.forEach(function(d){
  switch(d.class){
    case "hypothetical": 
    if(d.value>protein_class_max[0])protein_class_max[0]=d.value; break;
    case "infection": 
    if(d.value>protein_class_max[1])protein_class_max[1]=d.value; break;
    case "assembly": 
    if(d.value>protein_class_max[2])protein_class_max[2]=d.value; break;
    case "replication":
    if(d.value>protein_class_max[3])protein_class_max[3]=d.value; break;
    case "packaging": 
    if(d.value>protein_class_max[4])protein_class_max[4]=d.value; break;
    case "lysis": 
    if(d.value>protein_class_max[5])protein_class_max[5]=d.value; break;
    case "regulation": 
    if(d.value>protein_class_max[6])protein_class_max[6]=d.value; break;
    case "immune": 
    if(d.value>protein_class_max[7])protein_class_max[7]=d.value; break;
    case "integration":
    if(d.value>protein_class_max[8])protein_class_max[8]=d.value; break;
    case "tRNA": 
    if(d.value>protein_class_max[9])protein_class_max[9]=d.value; break;
  }
})
...
// gradient color rect
let label = svg.append("g").attr("transform", "translate(20,"+(height.value/2+5)+")")
const defs = label.append("defs"); //append defs
const linearGradient = defs.selectAll("graDef") //append <linearGradient> in defs
.data(protein_class).enter()
  .append("linearGradient")
  .attr("id", function(d){return "gradient"+d}); //corresponding id "gradient" + the name of the protein class
linearGradient 
  .append("stop")
  .attr("offset", "0%") 
...
linearGradient
  .append("stop")
  .attr("offset", "100%")
  .attr("stop-color", function(d){return chooseColor(d,1)});
const rect = label.selectAll("graRec").data(protein_class).enter()
  .append("rect")
  .attr("height", y.bandwidth())
  .attr("width", "80")
  .style("fill", function(d){return "url('#gradient"+d+"')"})
  .attr("y", function(d) {return y(d);})
const gratest = label.selectAll("graText").data(protein_class).enter()
gratest.append("text")
  .text(function(d){return protein_class_max[_.indexOf(protein_class,d)]})
  .style("fill", "white")
  .style("font-size", 10)
  .attr("x",75).attr("y", function(d) {return y(d)+y.bandwidth()/2+5;}).attr("text-anchor","end")
...
.append("circle")
    ...
    .attr("cx", function(d) { return x(d.id)+x.bandwidth()/2+leftMargin.value; })
    .attr("cy", function(d) { return y(d.class)+y.bandwidth()/2; })
    .attr("r", radius*2)
    .style("fill-opacity", 0)
    .style("stroke", "#282828")
...
// pies in the first graph 
  ...
  .append("circle")
    ...
    .style("stroke-dasharray", function(d) {return _.toString(d.perc/100*perimeter)+' '+_.toString(perimeter);})
...
svg.selectAll("myRec")
  ...
  .append("rect")
    .attr('class','myRec')
    .attr("x", function(d) { return x(d.id)+leftMargin.value+1; })
    .attr("y", function(d) { return y1(d.class);})
    .attr("width", x.bandwidth()-2)
    .attr("height", y1.bandwidth()-1)
    .style("fill", function(d) {return recColor(d)(d.value);} )
  .on("mouseover", mouseover)
  .on("mousemove", mousemove)
  .on("mouseleave", mouseleave)
})
